...
class TextDatabase(DocumentDatabase):
    ...
    def new_minutes(self, doc, minute_number):
        """
        Check if meeting already exists. 
        Otherwise, store a new meeting
        """
        # The root folder should already exist, (created during init)
        if os.path.isfile(f'{self.path}/minutes/{minute_number}/minutes.json'):
        ...
        os.makedirs(f'{self.path}/minutes/{minute_number}', exist_ok=True)
        with open(f'{self.path}/minutes/{minute_number}/minutes.json', 'w') as outfile:
    ...
    def delete_meeting(self, meeting_number):
        ...
        try:
            ...
            rmtree(f'{self.path}/minutes/{meeting_number}')
        except Exception as err:
    ...
    def retrieve_minutes(self, meeting_number):
        ...
        try:
            meeting_obj = self._convert_to_obj(meeting_dict, meeting_number)
        except Exception as err:
    ...
    def retrieve_proposal(self, minute_number, proposal_id):
        ...
        """
        if not os.path.exists(f'{self.path}/minutes/{minute_number}/minutes.json'):
            print('Meeting Does not exist')
            # Should Ideally also raise an Exception
            return None

        with open(f'{self.path}/minutes/{minute_number}/minutes.json', 'r') as infile:
            meeting_dict = json.load(infile)
    ...
    def _convert_to_obj(self, database_doc, meeting_number):
        """
        Internally convert to SenateMinutes() Format
        """
        # proposal_list = database_doc.keys()
        senate_obj = SenateMinutes('test', meeting_number)
